...
//
// focusSet (Part E4's weekly AI adjustment, see weeklyPlanAdjustments in
// db/schema.js): a SOFT bias, not a reorder of availableFromDay tiers --
// among subtopics that unlock at the same time, one the AI flagged as worth
// focusing on next sorts earlier. Never lets a focused subtopic jump ahead
// of one that's simply available sooner.
export function byScheduleOrder(focusSet) {
  return (a, b) =>
    a.availableFromDay - b.availableFromDay ||
    (focusSet?.has(b.id) ? 1 : 0) - (focusSet?.has(a.id) ? 1 : 0) ||
...
// focusSubtopicIds: Part E4's weekly nudge, see byScheduleOrder above.
export function assignLearningDays(subtopics, startDay = 0, focusSubtopicIds = []) {
  const focusSet = new Set(focusSubtopicIds);
  const byId = new Map(subtopics.map((s) => [s.id, s]));
  // Cross-chapter prerequisites (subtopics.prerequisiteSubtopicIds, see
  // lib/ai/generateSubjectBookPlan.js) are sanitized HERE, not trusted from
  // the caller -- AI-produced edges can reference an id outside this pool
  // or, worse, form a cycle; neither may ever be able to stall or
  // infinite-loop this scheduler.
  const prereqsOf = new Map();
  for (const s of subtopics) {
    const raw = Array.isArray(s.prerequisiteSubtopicIds) ? s.prerequisiteSubtopicIds : [];
    prereqsOf.set(s.id, raw.filter((id) => id !== s.id && byId.has(id)));
  }

  const remaining = new Set(subtopics.map((s) => s.id));
  const assignment = new Map();
  let day = startDay;

  // Round-robin across subjects (subjectId, e.g. gs1/gs2/political-science-
  // optional) -- a student with several subjects unlocked at once (the
  // common case: 2 GS papers + 1 optional, all availableFromDay 0) should
  // progress through all of them "at the same pace," not have one flat sort
  // exhaust an entire subject's queue before the others are ever touched.
  // First-appearance order, stable across calls given the same input.
  const subjectOrder = [];
  for (const s of subtopics) if (!subjectOrder.includes(s.subjectId)) subjectOrder.push(s.subjectId);
  let rotation = 0;

  // A priority-topological walk: among subtopics not yet scheduled, only
  // ones whose every prerequisite is already scheduled are "ready" this
  // round; byScheduleOrder breaks ties among those. Cycle safety: if
  // NOTHING is ready (every remaining subtopic is blocked, directly or via
  // a cycle in the AI-produced edges), fall back to scheduling the
  // byScheduleOrder-first remaining one anyway, ignoring its unresolved
  // prerequisites -- a stalled scheduler is worse than occasionally
  // ignoring a bad dependency edge.
  while (remaining.size) {
    let ready = [...remaining].filter((id) => prereqsOf.get(id).every((p) => !remaining.has(p)));
    if (!ready.length) ready = [...remaining];
    const readySubtopics = ready.map((id) => byId.get(id));

    // Prefer the next subject in rotation that actually has a ready item
    // this round (byScheduleOrder breaks the tie within that subject),
    // cycling through every subject present rather than always taking
    // byScheduleOrder's global top pick -- that's what previously let one
    // subject's whole queue run to completion before another subject's
    // first item was ever reached, purely by tiebreak accident (subtopic
    // ids happen to sort GS1 before GS2 before an optional's prefix).
    let s = null;
    for (let i = 0; i < subjectOrder.length; i++) {
      const candidateSubject = subjectOrder[(rotation + i) % subjectOrder.length];
      const candidates = readySubtopics.filter((c) => c.subjectId === candidateSubject);
      if (candidates.length) {
        s = candidates.sort(byScheduleOrder(focusSet))[0];
        rotation = (rotation + i + 1) % subjectOrder.length;
        break;
      }
    }
    if (!s) s = readySubtopics.sort(byScheduleOrder(focusSet))[0]; // defensive, should be unreachable
    remaining.delete(s.id);

    const prereqEndDays = prereqsOf.get(s.id).map((p) => assignment.get(p)?.endDay).filter((d) => d != null);
    let d = Math.max(day, s.availableFromDay, ...(prereqEndDays.length ? [Math.max(...prereqEndDays) + 1] : []));
 ...
 *
 * adjustment (Part E4, optional): { focusSubtopicIds, extraRevisionSubtopicIds }
 * from the latest weeklyPlanAdjustments row -- a soft bias on top of the
 * deterministic sort/selection below, never a replacement for it. Omit (or
 * pass null) for the exact pre-E4 behavior.
 */
export function buildDayPlan(subtopics, { fromDay, toDay, startDay = 0, adjustment = null }) {
  const learnDayBySubtopic = assignLearningDays(subtopics, startDay, adjustment?.focusSubtopicIds);
  ...
  for (let day = fromDay; day <= toDay; day++) {
    ...
    if (type === "learn") {
      ...
      // Part E4: extraRevisionSubtopicIds go first (still mastery-sorted
      // among themselves), then the rest of the usual lowest-mastery sort
      // fills any remaining slots -- a bias, not a replacement of the
      // underlying selection.
      const extraIds = new Set(adjustment?.extraRevisionSubtopicIds ?? []);
      const prioritized = learnedSoFar.filter((s) => extraIds.has(s.id)).sort((a, b) => a.masteryScore - b.masteryScore);
      const rest = learnedSoFar.filter((s) => !extraIds.has(s.id)).sort((a, b) => a.masteryScore - b.masteryScore);
      const subtopicIds = [...prioritized, ...rest].slice(0, REVISE_TOPICS_PER_DAY).map((s) => s.id);
      days.push({ day, type, subtopicIds });
    }
  }
