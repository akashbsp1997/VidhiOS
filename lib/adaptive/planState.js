...
import { eq, inArray, and, lte, desc } from "drizzle-orm";
import { db } from "../db.js";
import { subjectUnlocks, subjects, subtopics, mastery, sources, pyqs, playerState, weeklyPlanAdjustments } from "../../db/schema.js";
...
import { loadFixedWeekCompletion } from "./onboardingWeekState.js";

/**
 * The latest weekly nudge (Part E4) that applies to "now" -- the most recent
 * weeklyPlanAdjustments row with weekIndex <= the student's current week, so
 * a fresh adjustment keeps applying until the next one is written, and a
 * student with no adjustment yet (week 0, or the cron hasn't caught up)
 * simply gets null, which buildDayPlan treats as "no bias" (exact pre-E4
 * behavior).
 */
async function loadCurrentWeeklyAdjustment(userId, currentWeekIndex) {
  const [row] = await db
    .select()
    .from(weeklyPlanAdjustments)
    .where(and(eq(weeklyPlanAdjustments.userId, userId), lte(weeklyPlanAdjustments.weekIndex, currentWeekIndex)))
    .orderBy(desc(weeklyPlanAdjustments.weekIndex))
    .limit(1);
  return row ? { focusSubtopicIds: row.focusSubtopicIds, extraRevisionSubtopicIds: row.extraRevisionSubtopicIds, note: row.note } : null;
}
...
export async function getPlanWindow(userId, { fromDay, toDay }) {
  ...
  // Part E4: recomputed live every request (same philosophy as this file's
  // header comment about GS unlocks) rather than baked into a stored
  // schedule -- once the weekly cron writes a new row it takes effect on the
  // very next /api/plan call, and it applies as a bias on the WHOLE future
  // computed schedule (never the fixed week, see below) until superseded.
  const adjustment = await loadCurrentWeeklyAdjustment(userId, Math.max(0, Math.floor(todayDayNumber / 7)));
  ...
  if (fixedWeek) {
    ...
    const computed = buildDayPlan(remainingPool, { fromDay: Math.max(fromDay, TOTAL_FIXED_WEEK_DAYS), toDay, startDay: TOTAL_FIXED_WEEK_DAYS, adjustment });
    ...
    ({ days, learnDayBySubtopic } = buildDayPlan(ctx.subtopics, { fromDay, toDay, adjustment }));
  }

  const subtopicById = Object.fromEntries(ctx.subtopics.map((s) => [s.id, s]));

  // Whether each learn day's "write your answer" step is done -- an
  // attempts row for that subtopic created on that calendar day (see
  // loadFixedWeekCompletion, genuinely generic despite its name/home
  // module -- it was originally only called for the fixed onboarding
  // week's days, but its logic has no fixed-week-specific assumption in
  // it). Extended (per explicit request, "access previous day schedule
  // study and activities") to every learn day in the requested window, not
  // just the fixed week's -- previously a past day from the ongoing
  // computed engine (day 7+) always came back with no dayComplete/answered
  // at all, so looking back gave no sense of what was actually done that
  // day, only what had been scheduled. A day with zero scheduled topics is
  // excluded (nothing to mark complete), same as before.
  const learnDaysInWindow = days.filter((d) => d.type === "learn" && d.subtopicIds.length > 0);
  const completionByDay = learnDaysInWindow.length ? await loadFixedWeekCompletion(userId, ctx.planStartDate, learnDaysInWindow) : {};

  const enrichedDays = days.map((d) => {
    const completion = completionByDay[d.day];
    return {
      ...d,
      dayComplete: completion ? completion.complete : undefined,
      topics: d.subtopicIds.map((id) => {
        ...
        return {
          id,
          topicText: s?.topicText ?? id,
          subjectDisplayName: s?.subjectDisplayName ?? "",
          masteryScore: s?.masteryScore ?? 0,
          dayOfSpan,
          totalSpanDays,
          answered: completion ? completion.answeredSubtopicIds.includes(id) : undefined,
        };
      }),
    };
  });
  ...
  return {
    ...
    learnedSoFar,
    scheduledThroughDay,
    weeklyAdjustmentNote: adjustment?.note || null,
    days: enrichedDays,
  };
}
