...
export async function getPlanWindow(userId, { fromDay, toDay }) {
  ...
  // Part E3: whether each fixed-week learn day's "write your answer" step
  // is done -- an attempts row per required subtopic, created that calendar
  // day (see loadFixedWeekCompletion). Only meaningful for fixed-week learn
  // days (0..FIXED_WEEK_LEARN_DAYS-1); every other day gets no `dayComplete`/
  // `answered` keys at all (dropped by JSON.stringify), so the client can
  // tell "not tracked here" apart from "tracked and not done yet".
  const fixedWeekLearnDays = fixedWeek ? fixedWeek.days.filter((d) => d.type === "learn") : [];
  const completionByDay = fixedWeekLearnDays.length ? await loadFixedWeekCompletion(userId, ctx.planStartDate, fixedWeekLearnDays) : {};
