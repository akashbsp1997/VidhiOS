...
import { checkAndAwardOrnaments } from "../gamification/ornaments.js";
...
export async function applyGradedScore(userId, subtopicId, moduleId, score01) {
  ...
  // Ornaments (lib/gamification/ornaments.js) -- checked on every graded
  // attempt, not just on a fresh maturity transition, since a milestone
  // like "10 NCERT-grounded matures" can be crossed by ANY subtopic's
  // update, not only the one just graded. Internally non-fatal (never
  // throws), same "gamification must never break real grading" principle
  // as recordMissionSafe.
  await checkAndAwardOrnaments(userId);
