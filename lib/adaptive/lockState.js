...
import { computeDifficultyScore, orderSubtopicsWithinPaper, computeSubtopicLocks, computeSectionLocks } from "./unlocks.js";
...
export async function loadPaperLockMap(userId, subjectId, paper) {
  ...
  // Subject-level ("section") gate, composed with the subtopic-chain gate
  // above -- a subtopic is truly locked if EITHER says locked. Kept as a
  // distinct flag (lockedBySection) rather than collapsed silently into
  // `locked` so callers can tell a student *why* ("your previous chapter"
  // vs "your previous subject").
  ...
  for (const s of ordered) {
    ...
    // Deliberately does NOT override the section-level gate -- "early
    // access to a topic" means the topic itself opens, not a skip past its
    // whole subject the way a section gate would represent; see
    // lib/adaptive/unlocks.js's computeSectionLocks header comment.
    ...
    result.set(s.id, {
      ...lock,
      locked: chainLocked || sectionLock?.locked,
