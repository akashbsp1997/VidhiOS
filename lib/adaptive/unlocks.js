...
/**
 * Cross-chapter prerequisite locks -- independent of syllabusOrder position,
 * unlike computeSubtopicLocks' sequential chain: a subtopic can be locked by
 * this even if it sits EARLIER in syllabusOrder than its prerequisite (a
 * compound/synthesis chapter genuinely can be listed before the standalone
 * chapters it draws on). `subtopics` is [{id, prerequisiteSubtopicIds}].
 * Cycle-safe by construction: a subtopic is locked only while at least one
 * of its OWN listed prerequisites individually scores below threshold --
 * there's no traversal/recursion here, so a cycle in the AI-produced edges
 * (see lib/ai/generateSubjectBookPlan.js) can never hang this, only produce
 * a permanently-locked pair, an acceptable defensive degradation rather
 * than a crash. Returns Map<subtopicId, { locked, missingPrereqIds }>.
 */
export function computeCrossChapterLocks(subtopics, masteryScoreById, threshold = MASTERY_UNLOCK_THRESHOLD) {
  const map = new Map();
  for (const s of subtopics) {
    const prereqIds = Array.isArray(s.prerequisiteSubtopicIds) ? s.prerequisiteSubtopicIds.filter((id) => id !== s.id) : [];
    const missingPrereqIds = prereqIds.filter((id) => clamp01(masteryScoreById[id] ?? 0) < threshold);
    map.set(s.id, { locked: missingPrereqIds.length > 0, missingPrereqIds });
  }
  return map;
}
