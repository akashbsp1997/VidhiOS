...
Return ONLY a JSON object, no other text, in exactly this shape:
{
  "teachContent": "<600-1200 words of this module's explanation as a BULLETED list, not connected prose -- one clear, self-contained point per line, each line starting with '- ', separated by \\n (a single newline, not \\n\\n). Cover: what the concept is, its basis, how it works/developed, why it matters, how it plays out across different real question framings, and (where genuinely relevant) how current developments connect to it -- broken into scannable points a student can revise quickly rather than a wall of text.>",
  "keyPoints": [ "<a short, self-contained factual point worth remembering from this module. HARD CAP: 30 words.>" ]
}
Provide 5-10 keyPoints. Every field has a hard word/count cap stated above -- stop well before it, do not pad to fill it.`;
}
...
function buildModuleTeachUserPrompt({ subtopicText, moduleTitle, moduleScope, pyqQuestionText, sourceExcerpts, currentAffairsExcerpts, relatedPyqs }) {
  ...
  const grounding = sourceExcerpts && sourceExcerpts.length
    ...
    : "";
  const currentAffairs = currentAffairsExcerpts && currentAffairsExcerpts.length
    ? `\n\nReal current-affairs items tagged to this subtopic, weave in where genuinely relevant to this module's scope (do not force a connection that doesn't fit):\n${currentAffairsExcerpts.map((a) => `- ${a.title}: ${a.summary}`).join("\n").slice(0, 3000)}`
    : "";
  ...
  const related = relatedPyqs && relatedPyqs.length
    ? `\n\nReal exam questions asked on this subtopic (for context on the range of ways it's tested -- only cover the ones actually within this module's own scope, ignore the rest):\n${relatedPyqs.map((q) => `- (${q.marks} marks) ${q.questionText}`).join("\n")}`
    : "";
  return `Subtopic: ${subtopicText}
...
export function normalizeModuleTeachResult(raw) {
  ...
  return { teachContent: teachContent.slice(0, 8000), keyPoints };
}
