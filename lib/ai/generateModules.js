...
Provide 5-10 keyPoints and up to 5 currentAffairsLink points (fewer, or zero, if genuinely that's all that fits -- never pad). timelineEvents is SPECIAL: only fill it in when this module's real content is a genuine ORDERED SEQUENCE -- three kinds of module qualify: (a) a real historical chronology (dated events), (b) a staged/phased process with a fixed real order (e.g. India's three-stage nuclear power programme, a multi-step institutional process, a policy transmission mechanism like how a repo rate change moves through the banking system), or (c) an ordered list of a person's or institution's real achievements/milestones. Return an EMPTY array for any module that's fundamentally a single concept, a comparison, or an unordered set of facts, not a sequence (most modules aren't one). When you do fill it in, list steps in their real order, 3-8 of them, each grounded in the material -- never invent a stage or date you're not confident of. Every field has a hard word/count cap stated above -- stop well before it, do not pad to fill it.`;
}
...
export function normalizeModuleTeachResult(raw) {
  ...
  return { teachContent: teachContent.slice(0, 8000), keyPoints, currentAffairsLink, timelineEvents };
}
