...
Return ONLY a JSON object, no other text, in exactly this shape:
{
  ...
  "timelineEvents": [ { "date": "<a short date/year label, e.g. '1773' or '1858-1947'>", "label": "<what happened, max 10 words>" } ]
}
Provide 5-10 keyPoints and up to 5 currentAffairsLink points (fewer, or zero, if genuinely that's all that fits -- never pad). timelineEvents is SPECIAL: only fill it in when this module's real content is a genuine chronological sequence (a history of how something evolved through real dated stages) -- return an EMPTY array for any module that isn't fundamentally a timeline (most modules aren't). When you do fill it in, list events in real chronological order, 3-8 of them, each grounded in a real date/period from the material -- never invent a date you're not confident of. Every field has a hard word/count cap stated above -- stop well before it, do not pad to fill it.`;
}
...
export function normalizeModuleTeachResult(raw) {
  ...
  const currentAffairsLink = Array.isArray(raw?.currentAffairsLink)
    ...
    : [];
  // Empty for the vast majority of modules (only genuinely chronological
  // content gets one -- see buildModuleTeachSystem) -- never padded/forced.
