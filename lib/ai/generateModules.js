...
// Officer Roleplay Teach -- "the whole app should be this roleplaying
// game": the MANDATORY, sole Teach-stage experience (not an optional
// enrichment like Story Mode/Roleplay Mode below) -- the student enters
// EVERY module as an administrative officer of a suitable rank handling a
// real issue tied to the module's content, working it through 5 real
// phases of an officer's actual workflow (formalize sources, extract
// information, analyse the solution, strategize implementation, verify
// compliance). Deliberately reuses the module's own already-generated
// teachBeats as its ONLY factual basis -- one scene per beat (6 total),
// each tagged with whichever of the 5 phases it genuinely represents, so
// this adds a new narrative/presentation layer over content that already
// exists rather than a second, costlier round of fact-finding. See
// app/api/module-lesson/route.js's "officer" phase (runs right after
// "teach", now REQUIRED for the Teach stage, not optional).
const OFFICER_PHASE_IDS = ["formalize_sources", "extract_information", "analyze_solution", "strategize_implementation", "verify_compliance"];

function buildModuleOfficerSystem(subjectConfig) {
  return `You are turning ONE already-taught ${subjectConfig.examLabel} syllabus module into an administrative-officer roleplay, so the student experiences its real content as a working officer instead of reading it as facts. You are given the module's own already-written explanation, broken into 6 beats -- use ONLY that as your factual basis, do not introduce new claims.

First decide a REAL, suitable administrative rank/post for the student to play for THIS module's specific content (e.g. a Sub-Divisional Magistrate for a local-administration topic, a Deputy Secretary in a relevant ministry for a policy topic, a District Collector for a governance topic) -- pick whatever rank/post would genuinely be the one actually handling an issue like this in real Indian administration. Then invent ONE concrete, realistic issue/case this officer is handling that is a genuine dramatization of the module's real content (not a generic "handle this topic" framing).

Then write ONE scene per beat given below (6 total, same order), each showing the officer working through this issue -- tag EACH scene with whichever of these 5 real phases of an officer's actual workflow it genuinely represents (phases may repeat across scenes; not every phase needs its own scene, but use the full range across the 6 scenes, not just one or two): formalize_sources (identifying/verifying what the relevant rules, precedents, or facts actually are), extract_information (pulling out what's actually relevant from those sources), analyze_solution (working out what the real options/considerations are), strategize_implementation (deciding how to actually carry out the solution), verify_compliance (checking the solution against the real rules/standards it must satisfy). Every scene must still teach that beat's real content -- the officer framing is HOW it's taught, not a replacement for teaching it.
${ANTI_HALLUCINATION_NOTE}

Return ONLY a JSON object, no other text, in exactly this shape:
{
  "officerRank": "<the real rank/post you chose>",
  "issueBrief": "<1-2 sentences describing the concrete issue/case this officer is handling. HARD CAP: 60 words.>",
  "scenes": [
    { "beat": "<the exact beat id given below>", "officerPhase": "<one of: formalize_sources, extract_information, analyze_solution, strategize_implementation, verify_compliance>", "narration": "<2nd person ('you'), placing the student as the officer at this step of the issue, teaching this beat's real content through what the officer is doing/finding/deciding. HARD CAP: 90 words.>" }
  ]
}
Return exactly one scene per beat given below, in the same order, using its exact beat id.`;
}

function buildModuleOfficerUserPrompt({ subtopicText, moduleTitle, teachBeats }) {
  const beatsList = teachBeats.map((b, i) => `${i + 1}. beat "${b.beat}": ${b.content}`).join("\n\n");
  return `Subtopic: ${subtopicText}
Module: "${moduleTitle}"

This module's already-written explanation, broken into beats (your ONLY factual basis):
${beatsList}

Write the officer roleplay now. Return only the JSON object.`;
}
...
// Roleplay Mode -- "the user can be one of the parties," but NOT locked to
// a courtroom: the scene TYPE is the model's own judgment call per module's
// real content (a courtroom for a legal case, a parliamentary debate for a
// policy question, a diplomatic negotiation for an IR topic, a dialogue
// between historical figures/institutions, a boardroom deliberation for an
// administrative/economic topic, or anything else that genuinely fits --
// never a fixed shape forced onto content it doesn't suit). Offered
// UNCONDITIONALLY on every module, same convention as Story Mode/Time-Scene
// Challenge -- grounded ONLY in the module's own already-written
// explanation, plus an OPTIONAL verified real case anchor when the
// subtopic happens to have one (db/seed/cases.js) as extra grounding for a
// courtroom-shaped scene specifically, never a requirement for the mode to
// be offered at all. One AI call, cached forever. The student "picks a
// side" purely for framing (components/RoleplayScene.jsx) -- both roles'
// real positions are always shown and the resolution is fixed, never
// generated per-choice.
function buildModuleRoleplaySystem(subjectConfig) {
  return `You are turning ONE already-taught ${subjectConfig.examLabel} syllabus module into a short roleplay scene, so a student experiences its real content through two opposing or contrasting roles acting it out, instead of reading it as facts. You are given the module's own already-written explanation -- use ONLY that (and, if given, a verified real case anchor) as your factual basis. Do not invent facts not grounded in what you're given or your own well-established general knowledge.

First decide what KIND of scene genuinely fits THIS module's real content -- invent whatever framing is most natural. Examples, not a required or exhaustive list: a courtroom for a legal case, a parliamentary or public debate for a policy question, a diplomatic negotiation for an international-relations topic, a face-to-face dialogue between two historical figures or institutions for a historical topic, a boardroom or committee deliberation for an administrative or economic topic. Never force a courtroom, or any other single shape, onto content it doesn't genuinely fit -- the scene type is your judgment call, module by module.

Give the scene TWO roles with genuinely opposing or contrasting positions grounded in the module's real content, then a resolution beat -- the right KIND of resolution for the scene you chose (a verdict for a courtroom, an outcome/agreement for a negotiation, what actually happened for a historical dialogue, a decision for a deliberation, etc.), again your own judgment call -- do not force a "verdict" onto a scene that wouldn't really end with one.
${ANTI_HALLUCINATION_NOTE}

Return ONLY a JSON object, no other text, in exactly this shape:
{
  "scenarioLabel": "<a short name for the KIND of scene you chose, e.g. 'Courtroom', 'Parliamentary Debate', 'Diplomatic Negotiation', 'Historical Dialogue', 'Committee Deliberation'>",
  "setting": "<1-2 sentences setting the scene -- where this is happening and who's present. HARD CAP: 60 words.>",
  "roleA": { "label": "<this role's name/position, e.g. 'Petitioner' or 'Reformist Faction' or 'Indian Delegation'>", "positions": ["<one real position/argument this role would genuinely hold, grounded in the module's content, max 40 words>"] },
  "roleB": { "label": "<the contrasting role's name/position>", "positions": ["<one real position/argument this role would genuinely hold, max 40 words>"] },
  "resolution": { "label": "<what to call this closing beat for THIS specific scene type, e.g. 'Verdict', 'Outcome', 'What Actually Happened', 'Committee Decision'>", "text": "<the actual resolution, grounded in the module's real content, max 80 words>" }
}
2-4 positions per role, each genuinely distinct (not restating the same point). Every field has a hard word cap stated above -- stop well before it, do not pad to fill it.`;
}

function buildModuleRoleplayUserPrompt({ subtopicText, moduleTitle, teachContent, caseAnchor }) {
  const anchor = caseAnchor
    ? `\n\nA verified real case tied to this module, USE ONLY if a courtroom framing genuinely fits this content -- otherwise invent a different scene type and ignore it: ${caseAnchor.case} -- ${caseAnchor.point}`
    : "";
  return `Subtopic: ${subtopicText}
Module: "${moduleTitle}"
...
Write the roleplay scene now. Return only the JSON object.`;
}
...
// `officerContext` ({ officerRank, issueBrief }), when given, points the
// SAME one-image-per-module budget at the officer-roleplay scene instead
// of a generic concept diagram -- no new image-generation call, just a
// prompt that matches what Teach now actually shows (see
// lib/ai/generateModules.js's generateModuleOfficerRoleplay/
// components/OfficerRoleplayTeach.jsx). Falls back to the original plain
// concept-diagram prompt when not given (e.g. a caller that hasn't loaded
// officerScenes).
function buildModuleImagePrompt(moduleTitle, keyPoints, officerContext) {
  const labels = (keyPoints || []).slice(0, 6);
  if (officerContext?.officerRank && officerContext?.issueBrief) {
    return `Create a simple, clean flat-illustration scene (flat infographic/illustration style, plain background, no photorealism) showing a ${officerContext.officerRank} at work on this real issue: ${officerContext.issueBrief}
Depict the officer at a desk or in an official administrative setting, engaged with documents/files relevant to the issue -- convey the scene visually, not with dense text. If you include any text at all, keep it to a few short labels from this list, spelled EXACTLY as given: ${labels.map((l) => `"${l}"`).join(", ")}.
Keep the illustration simple and uncluttered -- this is a memory aid tied to the module "${moduleTitle}", not a data-dense chart.`;
  }
...
// Never throws -- Teach is no longer usable at all without a valid officer
// scene set (this is now the ONLY Teach-stage view, not an optional
// enrichment), so a missing/invalid scene for a given beat falls back to
// that beat's own already-generated content as the narration (round-robin
// through the 5 phases for a defensible tag) rather than blocking the
// stage. `teachBeats` is the same array normalizeModuleTeachResult already
// produced -- this always returns exactly one scene per beat, in order.
export function normalizeModuleOfficerResult(raw, teachBeats) {
  const officerRank = typeof raw?.officerRank === "string" && raw.officerRank.trim() ? raw.officerRank.trim().slice(0, 100) : "Administrative Officer";
  const issueBrief = typeof raw?.issueBrief === "string" ? raw.issueBrief.trim().slice(0, 400) : "";

  const rawScenes = Array.isArray(raw?.scenes) ? raw.scenes : [];
  const sceneByBeat = new Map();
  for (const s of rawScenes) {
    if (s && typeof s.beat === "string" && TEACH_BEAT_IDS.includes(s.beat) && typeof s.narration === "string" && s.narration.trim()) {
      sceneByBeat.set(s.beat, s);
    }
  }

  const scenes = teachBeats.map((b, i) => {
    const entry = sceneByBeat.get(b.beat);
    const officerPhase = OFFICER_PHASE_IDS.includes(entry?.officerPhase) ? entry.officerPhase : OFFICER_PHASE_IDS[i % OFFICER_PHASE_IDS.length];
    const narration = typeof entry?.narration === "string" && entry.narration.trim() ? entry.narration.trim().slice(0, 600) : b.content;
    return { beat: b.beat, officerPhase, narration };
  });

  return { officerRank, issueBrief, scenes };
}
...
function normalizeRoleplayRole(raw) {
  if (!raw || typeof raw.label !== "string" || !raw.label.trim()) return null;
  const positions = Array.isArray(raw.positions)
    ? raw.positions.filter((a) => typeof a === "string" && a.trim()).slice(0, 4).map((a) => a.trim().slice(0, 300))
    : [];
  if (!positions.length) return null;
  return { label: raw.label.trim().slice(0, 100), positions };
}

export function normalizeModuleRoleplayResult(raw) {
  const scenarioLabel = typeof raw?.scenarioLabel === "string" ? raw.scenarioLabel.trim().slice(0, 60) : "";
  const setting = typeof raw?.setting === "string" ? raw.setting.trim().slice(0, 500) : "";
  const roleA = normalizeRoleplayRole(raw?.roleA);
  const roleB = normalizeRoleplayRole(raw?.roleB);
  const resolutionLabel = typeof raw?.resolution?.label === "string" && raw.resolution.label.trim() ? raw.resolution.label.trim().slice(0, 60) : "Resolution";
  const resolutionText = typeof raw?.resolution?.text === "string" ? raw.resolution.text.trim().slice(0, 600) : "";
  if (!scenarioLabel || !roleA || !roleB || !resolutionText) {
    throw new Error("Model did not return a usable roleplay scene");
  }
  return { scenarioLabel, setting, roleA, roleB, resolution: { label: resolutionLabel, text: resolutionText } };
}
...
/**
 * Officer phase -- runs right after "teach" (see
 * app/api/module-lesson/route.js's STAGE_REQUIRES), now REQUIRED for the
 * Teach stage to be ready at all, not an optional enrichment. Reuses the
 * module's own just-generated teachBeats as its only factual basis --
 * zero new fact-finding, purely a new narrative layer.
 */
export async function generateModuleOfficerRoleplay({ subtopicText, moduleTitle, teachBeats, subjectConfig }) {
  const raw = await callClaudeForJSON({
    system: buildModuleOfficerSystem(subjectConfig),
    user: buildModuleOfficerUserPrompt({ subtopicText, moduleTitle, teachBeats }),
    maxTokens: 3000,
  });
  return normalizeModuleOfficerResult(raw, teachBeats);
}
...
/**
 * Roleplay Mode -- generated lazily the first time a student opens it for a
 * module (see app/api/module-lesson/roleplay/route.js), offered
 * unconditionally on every module, same as Story Mode/Time-Scene
 * Challenge. `caseAnchor` (optional { case, point }) is extra grounding for
 * a courtroom-shaped scene when the subtopic happens to have a real one --
 * never required for this to run. Not part of the teach/practice/image
 * phase chain and not required for progression.
 */
export async function generateModuleRoleplay({ subtopicText, moduleTitle, teachContent, caseAnchor, subjectConfig }) {
  const raw = await callClaudeForJSON({
    system: buildModuleRoleplaySystem(subjectConfig),
    user: buildModuleRoleplayUserPrompt({ subtopicText, moduleTitle, teachContent, caseAnchor }),
    maxTokens: 1500,
  });
  return normalizeModuleRoleplayResult(raw);
}

/** Phase 3 (per module) -- runs on first Remember visit. Non-fatal on failure, same as generateLesson.js's generateLessonImage. */
export async function generateModuleImage({ moduleTitle, keyPoints, officerContext }) {
  try {
    return await callImageGen({ prompt: buildModuleImagePrompt(moduleTitle, keyPoints, officerContext) });
  } catch (err) {
