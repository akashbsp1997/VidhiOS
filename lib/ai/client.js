...
async function callGemini({ system, user, maxTokens, apiKey, maxRetries, file }) {
  // `file` (legal document OCR/extraction, see lib/legal/extractDocument.js)
  // is an optional { mimeType, base64 } part appended alongside the text
  // turn -- Gemini reads an inlined image/PDF directly, no separate OCR step
  // needed. Every other caller (grading, question/lesson generation, ingest
  // structuring) omits it and gets the exact same text-only request as
  // before this existed.
  const userParts = [{ text: user }];
  if (file) userParts.push({ inlineData: { mimeType: file.mimeType, data: file.base64 } });
 ...
 */
async function callGeminiPool({ system, user, maxTokens, file }) {
  ...
  for (const apiKey of pool) {
    try {
      return await callGemini({ system, user, maxTokens, apiKey, maxRetries, file });
    } catch (err) {
      lastErr = err;
...
/**
 * Like callClaudeForJSON, but with an image/PDF inlined alongside the text
 * prompt for Gemini to read directly (OCR + understanding in one call) --
 * used by lib/legal/extractDocument.js to turn an uploaded legal document
 * into structured fields without a separate text-extraction pass. Gemini-
 * only: Groq serves no vision model, so there is no fallback tier here --
 * a Gemini-pool failure surfaces directly, same as callClaudeForJSON did
 * before Groq fallback existed.
 */
export async function callVisionForJSON({ system, user, file, maxTokens = 1500 }) {
  if (!file?.base64 || !file?.mimeType) {
    throw new AIRequestError("callVisionForJSON requires file.base64 and file.mimeType.");
  }
  const text = await callGeminiPool({ system, user, maxTokens, file });
  return parseJSONLoosely(text);
}
