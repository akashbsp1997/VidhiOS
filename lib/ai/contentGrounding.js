 ...
 * (NCERT, government/official, newspaper, other) instead of one flat,
 * undifferentiated blob -- so a generation prompt can actually see it has
 * material from multiple categories and is expected to draw on all of
 * them, not just whichever excerpt happened to sort first.
 *
 ...
 * just changes what each block IS (a whole tier's excerpts, labeled)
 * instead of one source row's raw text.
 */
export function labeledSourceExcerptBlocks(sourceRows) {
  const byTier = { ncert: [], official: [], newspaper: [], private_vendor: [] };
  for (const row of sourceRows) {
    ...
    const tier = byTier[row.sourceTier] ? row.sourceTier : "official";
