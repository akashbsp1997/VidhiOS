...
import { playerState, mastery, modelQuestions, pvpAttacks, allianceMembers } from "../../db/schema.js";
...
// Inlined rather than imported from ./alliances.js -- that module imports
// computeMatureCount/anonymizedLabel FROM this file, so importing back
// from it here would be a circular dependency. This is a 5-line query, not
// worth extracting a third shared module over.
async function sameAlliance(userIdA, userIdB) {
  const rows = await db
...
async function assertEligible(attackerUserId, defenderUserId) {
  ...
  if (await sameAlliance(attackerUserId, defenderUserId)) throw new Error("Can't attack an alliance member.");
