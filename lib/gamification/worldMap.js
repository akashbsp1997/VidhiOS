...
import { allianceTagFor } from "./alliances.js";
...
async function mapEntryFor(userId) {
  const [avgMastery, matureCount, badges, allianceTag] = await Promise.all([
    computeAvgMastery(userId),
    computeMatureCount(userId),
    listBadges(userId),
    allianceTagFor(userId),
  ]);
