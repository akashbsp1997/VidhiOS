...
/**
 * Spends exactly `amount` seeds, or throws and charges nothing --
 * distinct from deductSeeds' clamped, never-fails behavior (right for PvP
 * loot, where taking "whatever the loser has" is the point). A shop
 * purchase (lib/gamification/shop.js) is the opposite: charging less than
 * the agreed price for something the student keeps in full would be a real
 * bug, not a graceful degradation, so this fails loudly instead.
 */
export async function spendSeeds(userId, amount) {
  const [existing] = await db.select().from(playerState).where(eq(playerState.userId, userId));
  const current = existing?.seeds ?? 0;
  if (current < amount) throw new Error(`Not enough seeds -- need ${amount}, have ${current}.`);
  await db.update(playerState).set({ seeds: current - amount }).where(eq(playerState.userId, userId));
}
