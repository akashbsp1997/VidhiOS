...
import { buildOfficerTeachPanels } from "./OfficerRoleplayTeach.jsx";
...
export default function ModuleLearnFlow({ subtopicId, subjectDisplayName, subtopicText, initialData, initialModuleIndex = 0 }) {
  ...
  // Officer Roleplay Teach is now the SOLE Teach-stage view (full
  // replacement of the old bulleted/beat-panel rendering, per explicit
  // request -- "the whole app should be this roleplaying game"). "compact"
  // (Advanced mode) still skips straight to keyPoints for speed -- that's
  // an explicit power-user shortcut, not a form of the old bulleted view,
  // so it stays. The old "narrative" (Story mode) bulleted-vs-prose
  // distinction is gone: officer narration is already written as 2nd-person
  // prose, there's no separate bulleted form of it to toggle to.
  const teachPanels = moduleContent.teachContent
    ? [
        ...(mode.teachPrimary === "compact"
          ? []
          : buildOfficerTeachPanels({
              officerScenes: moduleContent.officerScenes,
              visualImageDataUri: moduleContent.visualImageDataUri,
