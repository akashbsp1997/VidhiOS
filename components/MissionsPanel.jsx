...
// Dashboard widget for the daily-missions/seeds/streak/inventory layer (see
...
//
// Seeds vs. XP: seeds are the spendable currency missions/mastering a topic
// earn (what this used to call "xp"); XP is now account age (days since
// signup) instead -- see lib/gamification/missions.js's header comment.
const LAST_SEEN_SEEDS_KEY = "vidhios-last-seen-seeds";
...
export default function MissionsPanel() {
  ...
  // Purely cosmetic "you earned seeds since you were last here" pulse --
  // this widget only fetches once on mount, it has no live channel to a
  // mission completing elsewhere in the app mid-session, so this compares
  // against the last value this browser saw (localStorage) rather than
  // claiming to detect the exact moment a mission completes.
  const [seedsJustIncreased, setSeedsJustIncreased] = useState(false);
      ...
      .then((data) => {
        if (!data.error) {
          ...
          if (typeof localStorage !== "undefined") {
            const lastSeen = Number(localStorage.getItem(LAST_SEEN_SEEDS_KEY) ?? 0);
            if (data.playerState.seeds > lastSeen) setSeedsJustIncreased(true);
            localStorage.setItem(LAST_SEEN_SEEDS_KEY, String(data.playerState.seeds));
          }
        }
      })
      .catch(() => {});
    ...
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 6 }}>
        ...
        <span style={{ fontSize: 12.5, color: "var(--ink-soft)" }}>
          🌱 <span className={seedsJustIncreased ? "value-pop" : undefined}>{playerState.seeds} seeds</span> · 🔥 {playerState.currentStreakDays}-day streak
          {playerState.longestStreakDays > playerState.currentStreakDays ? ` (best ${playerState.longestStreakDays})` : ""}
          {" · "}
          <span title="Time since your account was created">XP {playerState.xp}</span>
        </span>
      </div>
